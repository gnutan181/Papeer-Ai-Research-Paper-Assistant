...
def _reset_client() -> None:
    """Discard a broken HTTP connection pool before a transport retry."""
    global _client
...
def _upsert_batch_with_retry(
    ...
    """Retry only transient Qdrant transport failures.

    Point IDs are deterministic UUID5 values, so repeating an upsert after an
    uncertain disconnect overwrites the same points instead of duplicating them.
    """
    for attempt in range(1, UPSERT_MAX_ATTEMPTS + 1):
        try:
            ...
            print(
                "Warning: Qdrant disconnected while uploading "
                f"batch {batch_number}; reconnecting and retrying in "
                f"{delay_seconds}s (attempt {attempt + 1}/"
                f"{UPSERT_MAX_ATTEMPTS})."
            )
            _reset_client()
